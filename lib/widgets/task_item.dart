...
class _TaskItemState extends State<TaskItem> {
  ...
  Widget build(BuildContext context) {
    ...
    final TaskCategory? category = categoryProvider.getCategoryById(
    ...
    // 获取子任务
    final subTasks =
        widget.showSubTasks && widget.task.isMainTask
  ...
  Widget _buildTaskMetadata(ThemeData theme, TaskCategory? category) {
    return Wrap(
      ...
      children: [
        if (category != null)
          _buildMinimalTag(
            icon: CategoryIcons.fromCodePoint(category.iconCodePoint),
            label: category.name,
  ...
  Widget _buildDateTag(ThemeData theme) {
    ...
    Color color =
        isOverdue
            ? Colors.red
            : (isToday ? Colors.orange : theme.colorScheme.secondary);
