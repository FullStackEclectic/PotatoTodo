...
class _CategoryListScreenState extends State<CategoryListScreen> {
                ...
                children: [
                  ...
                  CategoryForm(
                    ...
                    onSave: (updatedCategory) async {
                      // 添加打印语句
                      debugPrint(
                        '[CategoryListScreen] onSave called. Category: ${updatedCategory.name}, ID: ${updatedCategory.id}',
                      );
                      ...
                      try {
                        ...
                        if (category == null) {
                          // 添加新分类
                          debugPrint(
                            '[CategoryListScreen] Adding new category...',
                          );
                          await categoryProvider.addCategory(updatedCategory);
                          debugPrint(
                            '[CategoryListScreen] Category added successfully.',
                          );
                        } else {
                          // 更新现有分类
                          debugPrint(
                            '[CategoryListScreen] Updating category...',
                          );
                          await categoryProvider.updateCategory(
                            updatedCategory,
                          );
                          debugPrint(
                            '[CategoryListScreen] Category updated successfully.',
                          );
                        }
                      } catch (e) {
                        debugPrint(
                          '[CategoryListScreen] Error saving category: $e',
                        );
                        // 可以在这里显示错误提示给用户
                        if (context.mounted) {
                          ScaffoldMessenger.of(
                            context,
                          ).showSnackBar(SnackBar(content: Text('保存分类失败: $e')));
                        }
                      }
  ...
  Future<void> _showDeleteConfirmation(
    ...
    if (confirmed == true && context.mounted) {
      if (category.id != null) {
        ...
        final categoryIds = categoryProvider.getCategoryTreeIds(category.id!);
        await categoryProvider.deleteCategory(category.id!);
        taskProvider.removeCategoriesFromTasks(categoryIds);
      }
    }
  }
  ...
  Widget _buildCategoryItem(
    ...
    final isExpanded =
        category.id != null && _expandedCategories.contains(category.id);
                  ...
                  if (!_isReordering) ...[
                    IconButton(
                      ...
                      onPressed: () {
                        ...
                        debugPrint(
                          '[CategoryListScreen] 进入排序模式，顶级分类数量: ${topLevelCategories.length}',
                        );
                        setState(() {
                          _isReordering = true;
                        ...
                        debugPrint(
                          '[CategoryListScreen] 排序列表初始化完成，数量: ${_reorderedCategories.length}',
                        );
                      },
                      tooltip: '排序分类',
                    ),
                    ...
                    IconButton(
                      ...
                      onPressed: () async {
                        ...
                        try {
                          await categoryProvider.updateCategoryOrder(
          ...
          if (_isReordering && _reorderedCategories.isEmpty) {
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(Icons.sort, size: 80, color: Colors.grey[400]),
                  const SizedBox(height: 16),
                  Text(
                    '没有可排序的分类',
                    style: TextStyle(fontSize: 18, color: Colors.grey[600]),
