...
class SmartInputParser {
  static ParsedTaskData parse(String input) {
    ...
    if (RegExp(r'(every day|daily|每天)').hasMatch(lowerTitleRef)) {
      ...
      repeatInterval = 1;
      title = _removePattern(
        title,
        RegExp(r'(every\s*day|daily|每天)', caseSensitive: false),
      );
      ...
      title = _removePattern(
        title,
        RegExp(r'(every\s*week|weekly|每周)', caseSensitive: false),
      );
      ...
      title = _removePattern(
        title,
        RegExp(r'(every\s*month|monthly|每月)', caseSensitive: false),
      );
    }
    ...
    if (lowerTitle.contains('tomorrow') ||
        lowerTitle.contains('tmr') ||
        lowerTitle.contains('明天')) {
      dueDate = now.add(const Duration(days: 1));
      title = _removePattern(
        title,
        RegExp(r'(tomorrow|tmr|明天)', caseSensitive: false),
      );
    } else if (lowerTitle.contains('today') || lowerTitle.contains('今天')) {
      dueDate = now;
      title = _removePattern(
        title,
        RegExp(r'(today|今天)', caseSensitive: false),
      );
    } else if (lowerTitle.contains('next week') || lowerTitle.contains('下周')) {
      dueDate = now.add(const Duration(days: 7));
      title = _removePattern(
        title,
        RegExp(r'(next\s*week|下周)', caseSensitive: false),
      );
    }
    // 4b. "In X days" / "X天后"
    else {
      final inDaysRegex = RegExp(r'(in|after)\s+(\d+)\s+(days?|d)|(\d+)(天后)');
      final match = inDaysRegex.firstMatch(
        lowerTitle,
      ); // Check lowercase for match
      if (match != null) {
