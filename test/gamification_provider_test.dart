...
void main() {
  ...
  group('GamificationProvider - Check-In Streak', () {
    ...
    test('First check-in should initialize streak to 1', () async {
      ...
      final streakBadge = provider.badges.firstWhere((b) => b.type == BadgeType.streakFire);
  ...
  group('GamificationProvider - Early Bird Streak', () {
    ...
    test('Task completed after 8 AM should not trigger early bird progression', () async {
      final provider = GamificationProvider();
      await Future.delayed(Duration.zero);
      ...
      final earlyBirdBadge = provider.badges.firstWhere((b) => b.type == BadgeType.earlyBird);
      expect(earlyBirdBadge.progress, 0);
    });

    test('First task completed before 8 AM should set morning streak to 1', () async {
      final provider = GamificationProvider();
      await Future.delayed(Duration.zero);
    ...
    test('Multiple tasks completed before 8 AM on same day should not double count', () async {
      final provider = GamificationProvider();
      await Future.delayed(Duration.zero);
    ...
    test('Task completed before 8 AM on consecutive day should increment morning streak', () async {
      SharedPreferences.setMockInitialValues({
    ...
    test('Task completed before 8 AM on non-consecutive day should reset morning streak to 1', () async {
      SharedPreferences.setMockInitialValues({
