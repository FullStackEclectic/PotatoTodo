...
void main() {
  group('CategoryProvider Tests', () {
    ...
    test('should create sub-categories correctly', () async {
      ...
      // 验证子分类
      final subCategories = categoryProvider.getSubCategories(workCategoryWithId.id!);
    ...
    test('should get category full name correctly', () async {
      ...
      expect(categoryProvider.getCategoryFullName(meetingCategoryWithId), '工作 > 会议');
    });
    ...
    test('should delete category with sub-categories correctly', () async {
      ...
      // 验证初始状态
      expect(categoryProvider.topLevelCategories.length, 1);
      expect(categoryProvider.getSubCategories(workCategoryWithId.id!).length, 1);
      ...
      expect(categoryProvider.getSubCategories(workCategoryWithId.id!).length, 0);
    ...
    test('should handle category hierarchy correctly', () async {
      ...
      // 创建多个层级的分类
      await categoryProvider.addCategory(TaskCategory(
        name: '工作',
        color: Colors.blue,
        iconCodePoint: Icons.work.codePoint,
        level: 0,
      ...
      expect(categoryProvider.getCategoryFullName(frontendCategory), '工作 > 项目 > 前端');
    });
  });
} 
